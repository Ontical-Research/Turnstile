...
test('header bottom edges are vertically aligned', async ({ page, mountApp }) => {
  ...
  const proofHeaderBottom = await page.evaluate(() => {
    // First child of the editor column — the PROOF header div
  ...
  console.log('PROOF header bottom:', proofHeaderBottom)
  console.log('PROOF ASSISTANT header bottom:', assistantHeaderBottom)
  console.log('Difference:', (proofHeaderBottom ?? 0) - (assistantHeaderBottom ?? 0))
